...
$pedidos_en_curso = mysqli_query($conexion, "SELECT * FROM pedidos WHERE status = 1");
$pedidos_completados = mysqli_query($conexion, "SELECT * FROM pedidos WHERE status = 0");
                    ...
                    <tr>
                        <td>
                            <select onchange="updateStatus(<?php echo $pedido['id']; ?>, this.value)">
                                <option value="1" <?php echo $pedido['status'] == 1 ? 'selected' : ''; ?>>Proceso</option>
                                <option value="0" <?php echo $pedido['status'] == 0 ? 'selected' : ''; ?>>Completado</option>
                            </select>
                        </td>
                        ...
                        <td><?php echo htmlspecialchars($pedido['salon']); ?></td>
  ...
  <script src="../js/custom.js"></script>
   ...
   <script>
    function updateStatus(pedidoId, newStatus) {
      $.ajax({
        url: 'update_status.php', // Archivo PHP que manejará la actualización
        method: 'POST',
        data: {
          id: pedidoId,
          status: newStatus
        ...
        success: function(response) {
          console.log(response); // Opcional: Muestra el resultado en la consola
